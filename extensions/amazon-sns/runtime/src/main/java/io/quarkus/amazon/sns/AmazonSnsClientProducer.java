...
import com.zappyride.quarkus.aws.config.*;
...
@ApplicationScoped
public class AmazonSnsClientProducer {
    private static final Log LOG = LogFactory.getLog(AmazonSnsClientProducer.class);

    @Inject
    private SnsServiceConfig runtimeConfig;

    private SnsClient client;
    private SnsAsyncClient asyncClient;
    
    @Inject
    private SnsBuildTimeConfig buildTimeConfig;
    ...
    public SnsClient client() {
        ...
        initSnsBaseClient(builder, runtimeConfig);
    ...
    private void initSnsBaseClient(SnsBaseClientBuilder builder, SnsServiceConfig config) {
        if(config == null)
            throw new RuntimeConfigurationError("The Configuration is null"+  System.getenv("AWS_ACCESS_KEY_ID"));
        ...
        if (config.credentials.type == AwsCredentialsProviderType.STATIC) {
            if (!config.credentials.staticProvider.accessKeyId.isPresent()
                ...
                throw new RuntimeConfigurationError(
                        "quarkus.sns.aws.credentials.static-provider.access-key-id and "
                                + "quarkus.sns.aws.credentials.static-provider.secret-access-key cannot be empty if STATIC credentials provider used.");
            }
        }
        if (config.credentials.type == AwsCredentialsProviderType.PROCESS) {
            if (!config.credentials.processProvider.command.isPresent()) {
                throw new RuntimeConfigurationError(
                        "quarkus.sns.aws.credentials.process-provider.command cannot be empty if PROCESS credentials provider used.");
            }
        }
...
//                    throw new RuntimeConfigurationError(
//                            String.format("quarkus.sns.sdk.endpoint-override (%s) - scheme must be specified",
...
//            }
        ...
        if(true){
            throw new RuntimeConfigurationError(config.toString());
        }
    ...
    private void initHttpClient(SnsClientBuilder builder, SyncHttpClientConfig config) {
    ...
    private void initHttpClient(SnsAsyncClientBuilder builder, NettyHttpClientConfig config) {
    ...
    private Builder createApacheClientBuilder(SyncHttpClientConfig config) {
    ...
    private void validateApacheClientConfig(SyncHttpClientConfig config) {
        if (config.apache.maxConnections <= 0) {
            throw new RuntimeConfigurationError("quarkus.sns.sync-client.max-connections may not be negative or zero.");
        }
    ...
    private void validateNettyClientConfig(NettyHttpClientConfig asyncClient) {
        if (asyncClient.maxConcurrency <= 0) {
            throw new RuntimeConfigurationError("quarkus.sns.async-client.max-concurrency may not be negative or zero.");
        }
        if (asyncClient.maxHttp2Streams < 0) {
            throw new RuntimeConfigurationError(
                    "quarkus.sns.async-client.max-http2-streams may not be negative.");
        }
        if (asyncClient.maxPendingConnectionAcquires <= 0) {
            throw new RuntimeConfigurationError(
                    "quarkus.sns.async-client.max-pending-connection-acquires may not be negative or zero.");
        }
        if (asyncClient.eventLoop.override) {
            if (asyncClient.eventLoop.numberOfThreads.isPresent() && asyncClient.eventLoop.numberOfThreads.get() <= 0) {
                throw new RuntimeConfigurationError(
                        "quarkus.sns.async-client.event-loop.number-of-threads may not be negative or zero.");
            }
        }
    ...
    private void validateProxyEndpoint(URI endpoint, String clientType) {
        ...
        if (StringUtils.isNotBlank(endpoint.getPath())) {
            throw new RuntimeConfigurationError(
                    String.format("quarkus.sns.%s-client.proxy.endpoint (%s) - path is not supported.",
                            clientType, endpoint.toString()));
        }
        if (StringUtils.isNotBlank(endpoint.getQuery())) {
            throw new RuntimeConfigurationError(
                    String.format("quarkus.sns.%s-client.proxy.endpoint (%s) - query is not supported.",
                            clientType, endpoint.toString()));
        }
